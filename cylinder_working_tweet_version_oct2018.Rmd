...
att_longer <- add_cyclic_data(gall_att) %>%
  filter(dates    >= ymd(20160515), 
         dates    <= ymd(20170802),
         cur_date <= ymd(20170802))

b <- ggplot(att_longer, aes(x_pos, `Gallery Attendance`, alpha = -distance, 
                            color = days_ago)) +
  geom_path(aes(size = -days_ago)) +
  # geom_vline(data = df_longer %>% filter(dates > max(dates) - cycle_length),
  #            aes(xintercept = x_pos), color = "gray50", alpha = 0.1) +
  
  geom_vline(data = att_longer %>% filter(month(dates) == 7, day(dates) == 1),
             aes(xintercept = x_pos, alpha = -distance), color = "gray90", size = 0.4) +
  
  geom_segment(data = att_longer %>% filter(day(dates) == 1, distance < -.4, x_pos < 0.5),
             aes(xend = x_pos, y = 0, yend = 500, alpha = -distance), 
             color = "gray90", size = 0.8) +
  
  geom_text(data = att_longer %>% filter(day(dates) == 1, distance < -.4, x_pos < 0.5),
             aes(y = 0, label = month(dates, label = T, abbr = T)), 
            size = 5, color = "red", hjust = 1.2) +
...
animate(b, nframes = 100, fps = 10, width = 300, height = 150) #, renderer = av_renderer())
